...
class Member extends CI_Controller {
	...
	// Simpan biaya admin asli di luar loop
	$biaya_admin_asli = $conf_bunga['biaya_adm'];
	...
	for ($i=1; $i <= $lama_ags; $i++) { $odat=array(); $biaya_admin_saat_ini=0; if ($i==1) {
		$biaya_admin_saat_ini=$biaya_admin_asli; } 
		
		$odat['angsuran_pokok']=number_format($angsuran_pokok); $odat['tgl_pinjam']=$tgl_pinjam;
		if($conf_bunga['pinjaman_bunga_tipe']=='A' ) { $biaya_bunga=($angsuran_pokok * $persen_bunga) / 100; } else {
		$biaya_bunga=($nominal * $persen_bunga) / 100; } 
		
		$odat['biaya_adm']=number_format($biaya_admin_saat_ini); $odat['biaya_bunga']=number_format($biaya_bunga); 
		
		$odat['jumlah_ags']=number_format($angsuran_pokok + $biaya_admin_saat_ini + $biaya_bunga);
		$tgl_tempo_var=substr($tgl_pinjam, 0, 7) . '-01' ; $tgl_tempo=date("Y-m-d", strtotime($tgl_tempo_var . " +" .$i."
		month")); $tgl_tempo=substr($tgl_tempo, 0, 7) . '-' . $denda_hari; $odat['tgl_tempo']=jin_date_ina($tgl_tempo);
		$out[]=$odat;
	 } 
		//var_dump($out); 
		if(!empty($out)) { echo '<h3>Simulasi Pinjaman</h3>' ;
		echo '<table class="table">' ; echo '  <tr>' ; echo '      <th style="text-align: center;">Ags Ke</th>' ;
		echo '      <th style="text-align: center;">Tanggal Tempo</th>' ;
		echo '      <th style="text-align: center;">Angsuran Pokok</th>' ;
		echo '      <th style="text-align: center;">Biaya Bunga</th>' ;
		echo '      <th style="text-align: center;">Biaya Admin</th>' ;
		echo '      <th style="text-align: center;">Jumlah Tagihan</th>' ; echo '  </tr>' ; $no=1; foreach ($out as $val) {
		echo '<tr>' ; echo '  <td style="text-align: center;">' .$no.'</td>';
		echo ' <td style="text-align: center;">'.$val['tgl_tempo'].'</td>';
		echo ' <td style="text-align: center;">'.$val['angsuran_pokok'].'</td>';
		echo ' <td style="text-align: center;">'.$val['biaya_bunga'].'</td>';
		echo ' <td style="text-align: center;">'.$val['biaya_adm'].'</td>';
