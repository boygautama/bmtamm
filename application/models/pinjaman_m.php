...
class Pinjaman_m extends CI_Model {
	...
	// {
	// 	$status = $this->input->post('aksi');
	// 	$id = $this->input->post('id'); // ID dari tbl_pengajuan
	// 	$alasan = $this->input->post('alasan');
	// 	$tanggal_u = date('Y-m-d H:i:s'); // Menggunakan format DATETIME

	// 	// Muat library session untuk mendapatkan username
	// 	$this->load->library('session');

	// 	switch ($status) {
	// 		case 'Hapus':
	// 			// Langsung hapus dan return
	// 			$this->db->where('id', $id);
	...
	// 			// 1. Ambil data lengkap dari pengajuan untuk dimasukkan ke tabel pinjaman
	// 			$pengajuan = $this->db->get_where('tbl_pengajuan', array('id' => $id))->row();

	// 			$sukubungas = $this->pinjaman_m->get_data_bunga('opsi_val');
	// 			$sukubunga = $sukubungas[0]->opsi_val;
	// 			$biayaadmins = $this->pinjaman_m->get_data_bunga('opsi_val');
	// 			$biayaadmin = $biayaadmins[0]->get_da
				

	// 			// Jika data pengajuan tidak ditemukan, hentikan proses
	// 			if (!$pengajuan) {
	// 				return FALSE;
	// 			}

	// 			// Ambil tanggal cair dari input
	// 			$tgl_cair = $this->input->post('tgl_cair');

	// 			// 2. Siapkan data untuk dimasukkan ke tbl_pinjaman_h
	// 			$data_pinjaman = array(
	// 				'tgl_pinjam' => $tgl_cair . ' ' . date('H:i:s'), // Gabungkan tgl cair dgn waktu sekarang
	// 				'anggota_id' => $pengajuan->anggota_id,
	// 				'lama_angsuran' => $pengajuan->lama_ags, // Asumsi ada kolom ini
	// 				'jumlah' => $pengajuan->nominal, // Asumsi nama kolomnya nominal
	// 				'bunga' => $sukubunga, // Asumsi ada kolom ini
	// 				'biaya_adm' => $pengajuan->biaya_adm, // Asumsi ada kolom ini
	// 				'lunas' => 'Belum', // Pinjaman baru pasti 'Belum' lunas
	// 				'dk' => 'D', // D = Debet (Pinjaman bertambah)
	// 				'kas_id' => $pengajuan->kas_id, // Asumsi ada kolom ini
	// 				'jns_trans' => $pengajuan->jns_trans, // Asumsi ada kolom ini
	// 				'update_data' => $tanggal_u,
	// 				'user_name' => $this->session->userdata('username'), // Ambil username dari session
	// 				'keterangan' => 'Pencairan dana dari pengajuan ID: ' . $id
	// 			);

	// 			// 3. Siapkan data untuk update di tbl_pengajuan
	// 			$data_update_pengajuan = array(
	// 				'status' => 1, // Status 'Setuju'
	// 				'alasan' => $alasan,
	// 				'tgl_cair' => $tgl_cair,
	// 				'tgl_update' => $tanggal_u
	// 			);

	// 			// 4. Gunakan Transaksi Database untuk keamanan data
	// 			$this->db->trans_start();

	// 			// Operasi 1: Masukkan data ke tabel pinjaman
	// 			$this->db->insert('tbl_pinjaman_h', $data_pinjaman);

	// 			// Operasi 2: Update status pengajuan
	// 			$this->db->where('id', $id);
	// 			$this->db->update('tbl_pengajuan', $data_update_pengajuan);

	// 			$this->db->trans_complete();

	// 			// Cek apakah transaksi berhasil atau tidak
	// 			if ($this->db->trans_status() === FALSE) {
	// 				// Transaksi gagal, kembalikan FALSE
	// 				return FALSE;
	// 			} else {
	// 				// Transaksi berhasil, kembalikan TRUE
	// 				return TRUE;
	// 			}
	...
	// 	// Blok ini sekarang hanya untuk case selain 'Setuju' dan 'Hapus'
	// 	$this->db->where('id', $id);
	// 	return $this->db->update('tbl_pengajuan', $simpan_arr);
	// }
	...
	public function create() {
		...
		$data = array(			
			...
			'bunga' => $this->input->post('bunga_txt'),
			'biaya_adm' => str_replace(',', '', $this->input->post('biaya_adm_txt')),
			'dk'						=>	'K',
			'jns_trans'				=>	'7',
